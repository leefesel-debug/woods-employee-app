...
function render(){
 ...
 $('#grid').innerHTML=shown.map(x=>{
  ...
  return `<article class="card"><div class="card-head"><div><h2>${safe(x.name)}</h2><div class="category">${safe(x.category||'Uncategorised')}</div></div></div><div class="dietary"><span class="diet vegan ${flags.vegan?'yes':'no'}">${flags.vegan?'✓ Vegan':'Not vegan'}</span><span class="diet gf ${flags.glutenFree?'yes':'no'}">${flags.glutenFree?'✓ Gluten free':'Not gluten free'}</span></div><div class="badges">${x.allergens?.length?x.allergens.map(y=>`<span class="badge">${safe(y)}</span>`).join(''):'<span class="badge none">No listed allergens</span>'}</div>${notes?`<p class="notes">${safe(notes)}</p>`:''}<div class="actions"><button class="mini" onclick="editItem('${x.id}')">Edit</button><button class="mini danger" onclick="archiveItem('${x.id}')">Archive</button></div></article>`;
 }).join('');
}
