...
function remoteShape(){return {
 ingredients:data.ingredients.map(i=>{const m=typeof editableCostMeta==='function'?editableCostMeta(i):{supplier:i.supplier||'',costBasis:i.costBasis||'',lastChecked:i.lastChecked||''};return {id:i.id,name:i.name,unit:i.unit||'each',pack_cost:i.packCost==null?null:Number(i.packCost),pack_qty:i.packQty==null?null:Number(i.packQty),active:i.active!==false,supplier:m.supplier||null,cost_basis:m.costBasis||null,last_checked:m.lastChecked||null}}),
...
async function upsertIngredients(rows){
 ...
 if(q.error&&cogsMetaSchemaReady&&(/supplier|cost_basis|last_checked|schema cache/i.test(q.error.message||''))){cogsMetaSchemaReady=false;q=await cogsDb.from('cogs_ingredients').upsert(legacyIngredients(rows));}
 return q;
}
async function cogsSeedRemote(){
 const s=remoteShape();
 let q=await upsertIngredients(s.ingredients);if(q.error)throw q.error;
...
async function cogsPushRemote(){
 ...
 try{
  ...
  cogsSyncing=false;if(cogsSyncPending){cogsSyncPending=false;setTimeout(cogsPushRemote,0)}
 }
}
...
async function cogsStart(){
 ...
 try{await cogsLoadRemote();installRemotePersistence();render();cogsBanner(cogsMetaSchemaReady?'Shared Supabase data connected · Admin':'Shared data connected · supplier metadata migration required',cogsMetaSchemaReady?'ok':'')}catch(e){console.error(e);cogsBanner('Supabase connection failed: '+(e.message||e),'error')}
}
