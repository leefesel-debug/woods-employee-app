...
let items=[],handbook=[],editing=null,currentUser=null,isAdmin=false;
...
async function applySession(session){
 ...
 await Promise.all([loadProducts(),loadHandbook()]);
}
...
function switchView(view){
 const handbookActive=view==='handbook';
 $('#allergenView').hidden=handbookActive;$('#handbookView').hidden=!handbookActive;
 $('#allergenTab').classList.toggle('active',!handbookActive);$('#handbookTab').classList.toggle('active',handbookActive);
 if(handbookActive){renderHandbook();window.scrollTo({top:0,behavior:'smooth'})}
}
...
$('#allergenTab').onclick=()=>switchView('allergen');$('#handbookTab').onclick=()=>switchView('handbook');
