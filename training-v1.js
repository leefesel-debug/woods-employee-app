let trainingRecords=[],editingTraining=null;
...
function trainingStatusLabel(status){return status==='expired'?'Expired':status==='due'?'Due soon':status==='current'?'Current':'No expiry'}
function updateTrainingHomeCount(){
 ...
 const action=trainingRecords.filter(x=>['expired','due'].includes(trainingStatus(x))).length;
...
function trainingRecordRow(item){
 ...
 return `<article class="training-record"><div class="training-record-main"><div class="training-course">${safe(item.training_name)}</div><div class="training-meta">Completed ${safe(trainingDate(item.training_date))}${item.expiry_date?' · expires '+safe(trainingDate(item.expiry_date)):' · no expiry'}</div>${detail?`<div class="training-detail">${safe(detail)}</div>`:''}</div><span class="training-status ${status}">${trainingStatusLabel(status)}</span><div class="training-admin-actions"><button class="mini" onclick="editTrainingRecord('${item.id}')">Edit</button><button class="mini danger" onclick="archiveTrainingRecord('${item.id}')">Archive</button></div></article>`;
}
function renderTraining(){
 ...
 const expired=trainingRecords.filter(x=>trainingStatus(x)==='expired').length,due=trainingRecords.filter(x=>trainingStatus(x)==='due').length;
 ...
 $('#trainingList').innerHTML=[...grouped.entries()].map(([name,records])=>{
  const roles=[...new Set(records.map(x=>x.role).filter(Boolean))].join(' / '),actions=records.filter(x=>['expired','due'].includes(trainingStatus(x))).length;
  return `<details class="training-person" ${open?'open':''}><summary><div><span class="training-person-name">${safe(name)}</span><span class="training-role">${safe(roles||'Role not recorded')}</span></div><div class="training-person-summary"><span>${records.length} record${records.length===1?'':'s'}</span>${actions?`<span class="training-status ${records.some(x=>trainingStatus(x)==='expired')?'expired':'due'}">${actions} due</span>`:''}</div></summary><div class="training-person-body">${records.map(trainingRecordRow).join('')}</div></details>`;
 }).join('');
}
...
async function importTraining(){
 ...
 try{
  ...
  const rows=source.map(x=>({...x,active:true,created_by:currentUser.id,updated_by:currentUser.id}));for(let i=0;i<rows.length;i+=100){const {error}=await db.from('training_records').insert(rows.slice(i,i+100));if(error)throw error}
...
function openTrainingEditor(item=null){
 if(!isAdmin)return;editingTraining=item?.id||null;$('#trainingDialogTitle').textContent=item?'Edit training record':'Add training record';
 const fields={trPerson:'person_name',trRole:'role',trCourse:'training_name',trDate:'training_date',trExpiry:'expiry_date',trDetails:'additional_details',trSigned:'signed_dated'};for(const [id,key] of Object.entries(fields))$('#'+id).value=item?.[key]||'';$('#trainingEditor').showModal();
}
window.editTrainingRecord=id=>openTrainingEditor(trainingRecords.find(x=>x.id===id));
async function saveTraining(e){
 e.preventDefault();if(!isAdmin)return;const value={person_name:$('#trPerson').value.trim(),role:$('#trRole').value.trim(),training_name:$('#trCourse').value.trim(),training_date:$('#trDate').value,expiry_date:$('#trExpiry').value||null,additional_details:$('#trDetails').value.trim(),signed_dated:$('#trSigned').value.trim(),updated_by:currentUser.id};
 const button=$('#saveTraining');button.disabled=true;button.textContent='Saving…';try{const result=editingTraining?await db.from('training_records').update(value).eq('id',editingTraining):await db.from('training_records').insert({...value,active:true,created_by:currentUser.id});if(result.error)throw result.error;$('#trainingEditor').close();await loadTraining()}catch(err){showMessage(err.message)}finally{button.disabled=false;button.textContent='Save record'}
}
...
$('#trainingSearch').oninput=renderTraining;$('#trainingStaff').onchange=renderTraining;$('#trainingStatus').onchange=renderTraining;$('#importTraining').onclick=importTraining;$('#addTraining').onclick=()=>openTrainingEditor();$('#trainingForm').onsubmit=saveTraining;
