...
Deno.serve(async (request) => {
  ...
  let orderId = ''
  try { orderId = String((await request.json()).order_id || '') } catch { return json({ error: 'Invalid request' }, 400) }
  ...
  if (!pendingOrder || !pendingOrder.active || pendingOrder.status !== 'prepared') {
    return json({ error: 'This order has already been sent, cancelled or is currently being processed' }, 409)
  }

  const { data: adminAccess } = await userClient.rpc('is_admin')
  if (pendingOrder.created_by !== user.id && adminAccess !== true) {
    return json({ error: 'Only the person who prepared this order or an administrator can send it' }, 403)
  }
  ...
  const { data: supplier } = await admin.from('order_suppliers')
    .select('id').eq('id', order.supplier_id).eq('name', order.supplier_name)
    .eq('email', order.supplier_email).eq('active', true).maybeSingle()
  if (!supplier) {
    await admin.from('purchase_orders').update({ status: 'prepared', updated_by: user.id, last_send_error: 'Supplier validation failed' }).eq('id', orderId)
    return json({ error: 'Supplier validation failed' }, 403)
  }

  const { data: lines, error: linesError } = await admin.from('purchase_order_lines')
    .select('supplier_item_id,quantity').eq('order_id', orderId).order('id')
  const itemIds = [...new Set((lines || []).map((line) => line.supplier_item_id).filter(Boolean))]
  const { data: catalogue, error: catalogueError } = itemIds.length
    ? await admin.from('order_items').select('id,supplier_id,singular_phrase,plural_phrase,active').in('id', itemIds)
    : { data: [], error: null }
  const items = new Map((catalogue || []).map((item) => [item.id, item]))
  const validLines = (lines || []).every((line) => {
    const item = items.get(line.supplier_item_id)
    return item && item.active && item.supplier_id === order.supplier_id && Number.isInteger(line.quantity) && line.quantity > 0 && line.quantity <= 99
  })
  if (linesError || catalogueError || !lines?.length || !validLines) {
    await admin.from('purchase_orders').update({ status: 'prepared', updated_by: user.id, last_send_error: 'Order item validation failed' }).eq('id', orderId)
    return json({ error: 'Order item validation failed' }, 400)
  }

  const orderLines = lines.map((line) => {
    const item = items.get(line.supplier_item_id)!
    return `- ${line.quantity} ${line.quantity === 1 ? item.singular_phrase : item.plural_phrase}`
  }).join('\n')
  const delivery = String(order.delivery_date)
  const deliveryDate = new Date(`${delivery}T12:00:00Z`)
  const deliveryLabel = Number.isNaN(deliveryDate.getTime())
  ...
  try {
    emailResponse = await fetch('https://api.resend.com/emails', {
      ...
      headers: {
        ...
        'Idempotency-Key': `woods-order-${order.id}`,
      },
      body: JSON.stringify({
    ...
    await admin.from('purchase_orders').update({ status: 'prepared', updated_by: user.id, last_send_error: 'Email provider could not be reached' }).eq('id', orderId)
    return json({ error: 'Email provider could not be reached' }, 502)
  }

  const providerData = await emailResponse.json().catch(() => ({}))
  if (!emailResponse.ok) {
    const detail = String(providerData.message || providerData.error || `Email provider returned ${emailResponse.status}`).slice(0, 500)
    await admin.from('purchase_orders').update({ status: 'prepared', updated_by: user.id, last_send_error: detail }).eq('id', orderId)
    return json({ error: detail }, 502)
  }

  const { error: updateError } = await admin.from('purchase_orders').update({
    status: 'sent', sent_at: new Date().toISOString(), updated_by: user.id,
    email_subject: subject, email_body: body,
    provider_message_id: providerData.id || null, last_send_error: null,
  }).eq('id', orderId).eq('status', 'sending')
  ...
  return json({ sent: true, order_id: orderId })
})
