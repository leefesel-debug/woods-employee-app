...
function orderMailto(order,email){const cc=order?.supplier_cc_email||orderSupplier?.cc_email,params=[];if(cc)params.push(`cc=${encodeURIComponent(cc)}`);params.push(`subject=${encodeURIComponent(email.subject||order.email_subject)}`);params.push(`body=${encodeURIComponent(email.body||order.email_body)}`);return`mailto:${encodeURIComponent(order?.supplier_email||orderSupplier.email)}?${params.join('&')}`}
...
async function prepareOrderEmail(){
 ...
 const supplierId=orderSupplier.id,mailto=orderMailto(value,email);await loadOrders(supplierId);window.location.href=mailto;
}
...
window.reopenOrderEmail=id=>{const order=purchaseOrders.find(x=>x.id===id);if(order)window.location.href=orderMailto(order,{})};
