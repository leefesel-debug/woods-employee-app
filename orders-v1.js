...
function orderMailto(order,email){const params=new URLSearchParams(),cc=order?.supplier_cc_email||orderSupplier?.cc_email;if(cc)params.set('cc',cc);params.set('subject',email.subject||order.email_subject);params.set('body',email.body||order.email_body);return`mailto:${encodeURIComponent(order?.supplier_email||orderSupplier.email)}?${params.toString()}`}
