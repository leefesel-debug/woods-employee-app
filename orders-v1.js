...
function renderRecentOrders(){
 ...
 $('#recentOrders').innerHTML=purchaseOrders.length?purchaseOrders.map(order=>{const lines=purchaseOrderLines.filter(x=>x.order_id===order.id);const mine=order.created_by===currentUser?.id;return`<article class="order-history-card"><div class="order-history-head"><div><span class="status-pill ${order.status==='cancelled'?'overdue':order.status==='prepared'?'due':''}">${orderStatusLabel(order.status)}</span><h3>${safe(order.supplier_name)}</h3><p>Delivery ${safe(orderDeliveryText(order.delivery_date))} · ${safe(new Intl.DateTimeFormat('en-GB',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}).format(new Date(order.created_at)))}</p></div><div class="order-history-actions"><button class="mini" onclick="reopenOrderEmail('${order.id}')">Open email</button>${order.status==='prepared'&&(mine||isAdmin)?`<button class="mini" onclick="markOrderSent('${order.id}')">Mark sent</button>`:''}${order.status!=='cancelled'&&(mine||isAdmin)?`<button class="mini danger" onclick="cancelOrder('${order.id}')">Cancel</button>`:''}</div></div><div class="order-line-summary">${lines.map(x=>safe(x.order_phrase)).join(' · ')||safe(order.email_body)}</div></article>`}).join(''):'<div class="handbook-empty">No Aubrey Allen orders have been prepared yet.</div>';
}
